<template>
  <div>
    <SfHeader :class="{ 'header-on-top': isSearchOpen, 'sf-header--has-mobile-search': !isMobileMenuOpen }"
      :is-nav-visible="isMobileMenuOpen">
      <!-- TODO: add mobile view buttons after SFUI team PR -->
      <template #logo>
        <nuxt-link :to="localePath('/')" class="sf-header__logo">
          <SfImage src="https://assets.g2g.com/ui/img/logo-white.png" :alt="$t('Vue Storefront Next')"
            class="sf-header__logo-image" />
        </nuxt-link>
      </template>
      <template #navigation>
        <div class="sf-header__navigation desktop" v-if="!isMobile">
          <SfHeaderNavigationItem v-for="category in headerNavigation" :key="category.name" class="nav-item"
            v-e2e="`app-header-${category.name}`" :label="category.name" :link="localePath(`/c/${category.link}`)" />
        </div>
        <SfModal v-else :visible="isMobileMenuOpen" :title="$t('Menu')" @close="toggleMobileMenu">
          <SfList>
            <SfListItem v-for="category in headerNavigation" :key="category.name"
              class="nav-item sf-header-navigation-item" v-e2e="`app-header-url_${category.link}`">
              <SfMenuItem :label="category.name" class="sf-header-navigation-item__menu-item"
                :link="localePath(`/c/${category.link}`)" @click.native="toggleMobileMenu" />
            </SfListItem>
          </SfList>
        </SfModal>
      </template>
      ...
      <template #header-icons>
        <div class="sf-header__icons">
          <SfButton v-e2e="'app-header-account'" class="sf-button--pure sf-header__action"
            aria-label="Open account button" @click="handleAccountClick">
            <SfIcon class="sf-header__icon" :icon="accountIcon" size="1.25rem" />
          ...
          <SfButton v-e2e="'app-header-cart'" aria-label="Toggle cart sidebar" class="sf-button--pure sf-header__action"
      ...
      <template #search>
        <SfSearchBar ref="searchBarRef" :placeholder="$t('Search for items')" :aria-label="$t('Search')"
...
export default {
  ...
  setup(props, { root }) {
    ...
    // handle stcky when scroll
    const handleScroll = () => {
      const header = document.querySelector('.sf-header__header');
      if (header) {
